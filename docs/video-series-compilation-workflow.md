...
| **Scene sections** | Grouped by category, each with `node generate-video.js "..."` command |
...
for ($i = 0; $i -lt $total; $i++) {
  ...
  node generate-video.js $clip.prompt --model grok --duration $clip.duration
}
...
node generate-music.js "epic sports compilation, orchestral swell, building energy, cinematic" --model music15 --duration 200

# Merge music under the compiled video
node generate-edit-video.js --op avmerge --video ./projects/<series>/media/compilation.mp4 --audio ./media/audio/<music-file>.mp3 --output ./projects/<series>/media/compilation-with-music.mp4
```
...
```powershell
# Generate narration
node generate-voice.js "Welcome to the gymnastics competition highlights reel..." --model elevenv3

# Merge voice over video
node generate-edit-video.js --op avmerge --video ./projects/<series>/media/compilation.mp4 --audio ./media/audio/<voice-file>.mp3 --output ./projects/<series>/media/compilation-voiced.mp4
```
...
for ($i=0; $i -lt $clips.Count; $i++) {
  node generate-video.js $clips[$i].prompt --model grok --duration $clips[$i].duration
}
